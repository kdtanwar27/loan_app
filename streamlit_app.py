...
# --- Input Fields ---
int_rate = st.number_input("Interest Rate (e.g., 0.11 for 11%)", min_value=0.0, max_value=1.0, step=0.01)
...
fico = st.number_input("FICO Credit Score", min_value=300, max_value=850, step=1)
...
revol_util = st.number_input("Revolving Utilization (%)", min_value=0.0, max_value=100.0)
...
pub_rec = st.number_input("Public Records (e.g., bankruptcies)", min_value=0, step=1)
not_fully_paid = st.radio("Loan Fully Paid?", options=[0, 1], format_func=lambda x: "Yes" if x == 0 else "No")
